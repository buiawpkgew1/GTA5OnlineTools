...
using GTA5OnlineTools.Views.Pages;
...
public partial class ThirdView : UserControl
{
    ...
    /// Third数据模型
    /// </summary>
    public ThirdModel ThirdModel { get; set; } = new();
    ...
    private void CheckThirdIsRun()
    {
        ...
        {
            // 判断 Kiddion 是否运行
            ThirdModel.KiddionIsRun = ProcessUtil.IsAppRun("Kiddion");
            // 判断 GTAHax 是否运行
            ThirdModel.GTAHaxIsRun = ProcessUtil.IsAppRun("GTAHax");
            // 判断 BincoHax 是否运行
            ThirdModel.BincoHaxIsRun = ProcessUtil.IsAppRun("BincoHax");
            // 判断 LSCHax 是否运行
            ThirdModel.LSCHaxIsRun = ProcessUtil.IsAppRun("LSCHax");
    ...
    private void ReadMeClick(string pageName)
    {
        ...
        {
            case "KiddionPage":
                ThirdModel.FrameState = Visibility.Visible;
                ThirdModel.FrameContent = KiddionPage;
                break;
            case "GTAHaxPage":
                ThirdModel.FrameState = Visibility.Visible;
                ThirdModel.FrameContent = GTAHaxPage;
                break;
            case "BincoHaxPage":
                ThirdModel.FrameState = Visibility.Visible;
                ThirdModel.FrameContent = BincoHaxPage;
                break;
            case "LSCHaxPage":
                ThirdModel.FrameState = Visibility.Visible;
                ThirdModel.FrameContent = LSCHaxPage;
                break;
            case "YimMenuPage":
                ThirdModel.FrameState = Visibility.Visible;
                ThirdModel.FrameContent = YimMenuPage;
                break;
        }
    }
    ...
    {
        ThirdModel.FrameState = Visibility.Collapsed;
        ThirdModel.FrameContent = null;
    }
}
